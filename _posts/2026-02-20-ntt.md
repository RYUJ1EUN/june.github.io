...
Let $\Z[x]$ and $\mathbb{Z}_q[x]$ be the polynomial rings over $\Z$ and $\mathbb{Z}_q$ respectively, with corresponding quotient rings $\Z[x]/\!\left<\phi(x)\right>$ and $\mathbb{Z}_q[x]/\!\left<\phi(x)\right>$, where $\phi(x)$ is a polynomial with integer coefficients.
...
If $\deg \phi(x)=n$ holds, the element in $\mathbb{Z}_q[x]/\!\left<\phi(x)\right>$, for example, $\boldsymbol a$, can be represented in the form of $\boldsymbol{a}=\sum_{i=0}^{n-1}a_i x^i$, or in the form of $\boldsymbol{a}=(a_0, a_1, \,...\,,a_{n-1})$`vector space over Zq`, where $a_i\in\mathbb{Z}_q$.
