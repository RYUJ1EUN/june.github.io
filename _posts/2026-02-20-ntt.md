...
- $x' = x \pmod q$: $x'$ is the unique element in $\Z_𝑞$ satisfying $𝑥' \equiv 𝑥 \pmod 𝑞$
- $\bf a$, $a(x)$: polynomials
- $\bf r=a\mod b$: 𝒓 is the polynomial remainder of 𝒂 divided by 𝒃
- For any $n_1,n_2\in\Z$, $n_1 > n_2$ and any $a_i$, $\sum_{j=n_1}^{n_2} a_i = 0$
...
The elements of the set $U_{k}=\left\{ \psi\in\mathbb{C} \mid \psi^k=1 \right\}$ are called the $k$-th roots of unity.
...
Notice that primitive and principal $𝑘$-th root of unity coincide if $R = \Z_q$ where $q$ is a prime number.

- **Coincide if $R=\Z_q$**
    1. Primitive ⇒ Principal
        
        $\Z_q$의 primitive $k$-th root of unity $\psi$가 존재하면, $\psi$는 $\Z_q$의 principal $k$-th root of unity이다.
    ...
    2. Principal ⇒ Primitive 
        ...
        $\Z_q$의 principal $k$-th root of unity $\psi$가 존재하면, $\psi$는 $\Z_q$의 primitive $k$-th root of unity이다.
        
        pf ) $\psi$가 $\Z_q$의 principal $k$-th root of unity일 때,
...
- Example
    ...
    2, 3 are primitive 4-th roots of unity in $\Z_5$
    ...
    2, 3 are principal 4-th roots of unity in $\Z_5$
    
    → primitive and principal 4-th roots of unity coincide in $\Z_5$
    

#### Polynomial Rings

Let $\Z[x]$ and $\Z_q[x]$ be the polynomial rings over $\Z$ and $\Z_q$ respectively, with corresponding quotient rings $\Z[x]/\!\left<\phi(x)\right>$ and $\Z_q[x]/\!\left<\phi(x)\right>$, where $\phi(x)$ is a polynomial with integer coefficients.
...
- Preliminary
    ...
    **Thm)** Eisenstein Criterion
    ...
    Let $q\in\Z$ be a prime.
    
    Suppose that $f(x)=a_nx ^n +\cdots+a_0$ is in $\Z[x]$, and $a_n\neq 0\pmod q$, but $a_i=0\pmod q$ for all $i<n$, with $a_0\neq 0\pmod {q^2}$.
...
If $\deg \phi(x)=n$ holds, the element in $\Z_q[x]/\!\left<\phi(x)\right>$, for example, $\boldsymbol a$, can be represented in the form of $\boldsymbol{a}=\sum_{i=0}^{n-1}a_i x^i$, or in the form of $\boldsymbol{a}=(a_0, a_1, \,...\,,a_{n-1})$`vector space over Zq`, where $a_i\in\Z_q$.

Main target rings are $\Z_q[x]/(x^n-1)$ and $\Z_q[x]/(x^n+1)$, since they are widely used in lattice-based schemes.
...
3. **Injective:** 
    ...
    4. $h(x)$가 $f(x)$의 배수라는 말은, $F[x]/\langle f(x) \rangle$)에서 $0$과 동치라는 뜻이다.
        ...
        $$
        f(x) = f_1(x)f_2(x)\cdots f_k(x) \;\mid\; h(x)\\[1em]
...
$$
A(x) \equiv a_i(x) \pmod{f_i(x)}\\[.5em]
...
2. 두 번째 조건 ($x^2+1$): $x^2 \equiv -1$ 대입
    ...
    $$
    A(x) \equiv -\frac{1}{2}(-1) + 2x + \frac{1}{2} = \frac{1}{2} + 2x + \frac{1}{2} = 2x + 1\\[.5em]
...
- **Linear** convolution
    ...
    $$
    \boldsymbol{c} = \sum_{k=0}^{2n-2} c_k x^k \in \mathbb{Z}_q[x],\\[.5em]
    \text{where }
...
- It implies that the primitive $n$-th root of unity $\omega_n$ in $\mathbb{Z}_q$ exists.
    ...
    따라서 $\Z_q$에는 order$\,=n$인 cyclic subgroup의 generator가 존재한다.
    
    order$\,=n$인 $x\in\Z_q$는 다음을 만족한다.
...
- the primitive $2n$-th root of unity $\psi_{2n}$ in $\mathbb{Z}_q$ exists.
    ...
    Negative Wrapped Convolution은 $\Z_q[x]/(x^n+1)$에서 연산을 수행하므로 $x^n \equiv -1$가 되는 $x$를 구해야 한다. 
    ...
    \psi_{2n}^{n}\equiv -1$인 primitive $2n$-th root of unity $\psi_{2n}\in\Z_q$를 구할 수 있다.
    
    이때, $\psi_{2n}$는 primitive이므로 $k<2n$에 대하여 $\psi_{2n}^k\neq 1$이므로 $\psi_{2n}^n\equiv1$이다.
    
    ※  $\mathbb{Z}_q[x]/(x^{2n}-1)$의 primitive $2n$-th roots of unity $\psi_{2n}$
...
- One-iteration Karatsuba algorithm saves one multiplication at the cost of three extra additions (subtractions).
    ...
    \begin{aligned}
    t_2 &= ad+bc & 
    \text{\quad 2 multiplications}\\
    &= ad+bc+ac-ac+bd-bd\\
    &= ac+bc+ad+bd-ac-bd & \downarrow\qquad\quad\,\\
    &= (a+b)(c+d)-ac-bd\\
    &= (a+b)(c+d)-t_1-t_2 & 
    \text{\quad 1 multiplication\,}
    \end{aligned}
    $$
...
$$
\mathbb{Z}_q[x]/(x^{h \cdot 2^k} - 1) \xrightarrow{\phi\,} \left(\mathbb{Z}_q[z]/(z^{2^k} - 1)\right)[y]/(y^h - 1), \\[1em]
\begin{aligned}
\text{where }\;
...
- 원리: 중국인의 나머지 정리
    ...
    $$
    \phi(x^l)= y^{(l \bmod h)} \!\cdot\! z^{(l \bmod 2^k)}\\[1em]
    a_l = \tilde{a}_{i,j}
    $$
...
#### [+] NUMBER THEORETIC TRANSFORM OVER $\Z_Q[X]/(X^N-x^{N/2} +1)$
...
##### Incomplete FFT trick over $\Z_q[x]/(x^n -x^{n/2} +1)$
...
- Actually, $x^n - x^{n/2} + 1$ is the $3n$-th cyclotomic polynomial of degree $n$, where $n = 3^l \cdot 2^e\; (l \ge 0, e \ge 1)$, not a power-of-two cyclotomic polynomial anymore.
    ...
    $$
    \Phi_{3n}(x) = x^n - x^{n/2} + 1\\[.5em]
    \Phi_{3n}(x) = \Phi_{6(n/2)}(x) =  \Phi_6(x^{n/2}) = (x^{n/2})^2 - (x^{n/2}) + 1\\[.5em]
...
$$
\mathbb{Z}_q[x]/(x^n - x^{n/2} + 1) \;\cong\; \mathbb{Z}_q[x]/(x^{n/2} - \zeta_1) \times \mathbb{Z}_q[x]/(x^{n/2} - \zeta_2),\\[.5em]
...
$$
\boldsymbol{a}_l = \boldsymbol{a} \bmod x^{n/2} - \zeta_1 \in \mathbb{Z}q[x]/(x^{n/2} - \zeta_1)\\[.5em]
\text{and}\quad
...
##### Splitting Polynomial Ring over $\Z_q[x]/(x^n -x^{n/2} +1)$
...
Similar to NTTRU, there is a CRT map as follows
...
The leaf nodes of the CRT tree map are degree-$2^\beta$ polynomials, e.g., $\mathbb{Z}_q[y]/(y^{2^\beta} - \psi_{n/2^{\alpha+\beta}}^j)$. 
...
for polynomial rings $\Z_q[x]/(x^{2m}-\omega^2)$ where $m>0$ and invertible $\omega\in\Z_q$,
...
![image]({{ '/assets/img/post_ntt/image.png' | relative_url }})
<!-- ![image.png](image.png) -->
...
![image]({{ '/assets/img/post_ntt/image_1.png' | relative_url }})
...
![image]({{ '/assets/img/post_ntt/image_2.png' | relative_url }})
...
![image]({{ '/assets/img/post_ntt/image_3.png' | relative_url }})
<!-- ![image.png](image%203.png) -->

Figure 2 shows the detailed process of using FFT trick to map $\mathbb{Z}_q[x]/(x^n - 1)$. 

![image]({{ '/assets/img/post_ntt/image_4.png' | relative_url }})
...
![image]({{ '/assets/img/post_ntt/image_5.png' | relative_url }})
<!-- ![image.png](image%205.png) -->

![image]({{ '/assets/img/post_ntt/image_6.png' | relative_url }})
...
![image]({{ '/assets/img/post_ntt/image_7.png' | relative_url }})
<!-- ![image.png](image%207.png) -->

Adjust its input/output order and we can obtain $\text{INTT}^{GS}_{no \to bo}$.

![image]({{ '/assets/img/post_ntt/image_8.png' | relative_url }})
<!-- ![image.png](image%208.png) -->

- $\text{NTT}^{GS}$
    
    
    ![image]({{ '/assets/img/post_ntt/image_9.png' | relative_url }})
    <!-- ![image.png](image%209.png) -->
    
    ![image]({{ '/assets/img/post_ntt/image_10.png' | relative_url }})
...
![image]({{ '/assets/img/post_ntt/image_11.png' | relative_url }})
...
![image]({{ '/assets/img/post_ntt/image_12.png' | relative_url }})
<!-- ![image.png](image%2012.png) -->

Similarly, by adjusting its input/output order, we can get $\text{NTT}^{CT, \psi}_{bo \to no}$.

![image]({{ '/assets/img/post_ntt/image_13.png' | relative_url }})
...
![image]({{ '/assets/img/post_ntt/image_14.png' | relative_url }})
<!-- ![image.png](image%2014.png) -->

Adjust the input/output order and get $\text{INTT}^{GS, \psi^{-1}}_{no \to bo}$.

![image]({{ '/assets/img/post_ntt/image_15.png' | relative_url }})
...
\\[2.5em]
&(\Psi\Phi)^{-1}\!\!\left(\sum_{i=0}^{ n/2\,-1\!\!} a'_i x^i, \sum_{i=0}^{ n/2\,-1\!\!} a''_i x^i\!\right) \\[1.5em] & =\  \sum_{i=0}^{ n/2\,-1\!} \frac{a'_i + \omega_n^{-i} \cdot a''_i}{2} x^i + \sum_{i=0}^{ n/2\,-1\!} \frac{a'_i - \omega_n^{-i} \cdot a''_i}{2} x^{i+{n}/{2} }.
\end{aligned}
$$
...
![image]({{ '/assets/img/post_ntt/image_16.png' | relative_url }})
<!-- ![image.png](image%2016.png) -->

Such GS NTT algorithm is denoted by $\text{NTT}^{GS}_{no \to bo}$. 

![image]({{ '/assets/img/post_ntt/image_17.png' | relative_url }})
<!-- ![image.png](image%2017.png) -->

Adjust the input/output order, and we obtain $\text{NTT}^{GS}_{bo \to no}$. 

![image]({{ '/assets/img/post_ntt/image_18.png' | relative_url }})
...
The primitive $n$-th root of unity $\omega_n$ in $\Z_q$ has the following properties

$$
\text{periodicity: }
\omega_n^{k+n} = \omega_n^k\\[.5em]
\text{symmetry: }
...
This section introduces the recent advances of weakening parameter restrictions with respect to $\Z_q[x]/(x^n\pm 1)$ where $n$ is a power of two. Those methods can be mainly classified into the following three categories.
...
![image]({{ '/assets/img/post_ntt/image_19.png' | relative_url }})
...
Fully-mapping FFT trick means to map $\mathbb{Z}_q[x]/(x^n + 1)$ down to linear terms, e.g., $\mathbb{Z}_q[x]/(x - \psi_{2n}^{2i+1})$. The condition $q \equiv 1 \pmod{2n}$ is required such that the primitive $2n$-th root of unity $\psi_{2n}$ exits. 
...
- **Step 3, Gatheration.**
    ...
    Gather all the $\tilde{c}_i$ by $\Psi_\alpha^{-1}$, and obtain $c = \Psi_\alpha^{-1} \left( \sum_{i=0}^{2^\alpha-1} \tilde{c}_i \cdot x^i \right)$.
...
For example, to compute $(\sum_{i=0}^{2^\beta-1} \hat{a}_i x^i)(\sum_{i=0}^{2^\beta-1} \hat{b}_i x^i) \pmod{x^{2^\beta} - \psi}$, 
...
More specially, $\text{NTT}^{CT, \psi}_{no \to bo}$ and $\text{INTT}^{GS, \psi^{-1}}_{bo \to no}$ are the cases of H-NTT over $\mathbb{Z}_q[x]/(x^n + 1)$ when $\alpha = 0, \beta = 0$, 

and $\text{NTT}^{CT}_{no \to bo}$ and $\text{INTT}^{GS}_{bo \to no}$ are the cases of H-NTT over $\mathbb{Z}_q[x]/(x^n - 1)$ when $\alpha = 0, \beta = 0$.
...
Consider $\mathbb{Z}_N[x]/(x^n + 1)$, where $N = \prod_{i=1}^l q_i$ and each $q_i$ is NTT-friendly prime. If $2n \mid \gcd(q_1 - 1, \dots, q_l - 1)$, there exits a principal $2n$-th root of unity $\psi_{2n}$ in $\mathbb{Z}_N$. 

$\psi_{2n}$ is a principle $2n$-th root of unity in $\mathbb{Z}_N$, iff $(\psi_{2n} \bmod q_i)$ is a principle $2n$-th root of unity in $\mathbb{Z}_{q_i}$ for any $i$. 

$$
\mathbb{Z}_N[x]/(x^n + 1) \cong \prod_{i=0}^{n-1} \mathbb{Z}_N[x]/(x - \psi_{2n}^{2brv_n(i)+1}).
$$

Its forward transform and inverse transform are illustrated as $\text{NTT}^{CT, \psi}_{no \to bo}$ and $\text{INTT}^{GS, \psi^{-1}}_{bo \to no}$ over $\mathbb{Z}_N[x]/(x^n + 1)$. 
...
$\omega_n$ is a principle $n$-th root of unity in $\mathbb{Z}_N$, iff $(\omega_n \bmod q_i)$ is a principle $n$-th root of unity in $\mathbb{Z}_{q_i}$ for any $i$. 
...
| --- | --- | --- | --- | --- | --- |
| Kyber  Round 1 | 256 | 7681 | $\mathbb{Z}_q[x]/(x^n + 1)$ | NTT-friendly 
$q \equiv 1 \pmod{2n}$ | $n$-point full NWC-based NTT |
| Kyber  Round 2, Kyber  Round 3 | 256 | 3329 | $\mathbb{Z}_q[x]/(x^n + 1)$ | NTT-friendly 
$q \equiv 1 \pmod{n}$ | Incomplete FFT trick,
Splitting polynomial ring |
| Dilithium Round 3 | 256 | 8380417 | $\mathbb{Z}_q[x]/(x^n + 1)$ | NTT-friendly  
$q \equiv 1 \pmod{2n}$ | $n$SS-point full NWC-based NTT |

![image]({{ '/assets/img/post_ntt/image_20.png' | relative_url }})
...
The point-wise multiplication is treated as the corresponding polynomial multiplications in $\mathbb{Z}_q[x]/(x^2 - \omega_{n}^{2brv_{n/2}(i)+1})$. 
