...
- Preliminary
    ...
    **Cor)** The polynomial
    ...
    $$
    \phi_n(x)=\prod_{1\le k<n,
    $$
    $$
    \gcd(k,n)=1}(x-e^{2\pi ki/n})
    $$
...
If $\deg \phi(x)=n$ holds, the element in $\mathbb{Z}_q[x]/\!\left<\phi(x)\right>$, 
for example, $\boldsymbol a$, can be represented in the form of 
$\boldsymbol{a}=\sum_{i=0}^{n-1}a_i x^i$, or in the form of 
$\boldsymbol{a}=(a_0, a_1, \,...\,,a_{n-1})$`vector space over Zq`, 
