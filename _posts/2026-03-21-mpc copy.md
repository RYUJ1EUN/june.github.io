...
| $k$-bit | | $k \xrightarrow{\text{linear}} n$ | | $n$-bit$|
| | | $[n, k]\_2$ code | | |
| | | $C[n, k]$: linear codeword | | $\downarrow\leftarrow \text{error}|
| | | $k < n$ (오류를 정정하기 위함) | | |
...
- $|C| = 2^k \qquad \because$ encode; linear $1-1$(복원해야 하므로) function
...
- linear code는 $$\min\_{{\bf u} \neq {\bf v}} d\_H({\bf u}, {\bf v}) = \min\_{{\bf u} \neq {\bf v}} w\_H({\bf u} - {\bf v}) = \min\_{{\bf u} \in C \setminus \{0\}} w\_H {\bf u}$$
...
$(1^T, 2^T, \dots, n^T) = H = (h\_1, h\_2, \dots, h\_n)$ 이라고 하면 
...
* $H$ 생성
    ...
    - $x\_i \in \{0, 1\}$ 이므로 오류 발생 시 $h\_i$ 만큼 신드롬이 나타남 ($Hx + h\_i = 0 + h\_i = h\_i$)