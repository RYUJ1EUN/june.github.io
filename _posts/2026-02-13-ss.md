...
1. $D$ secretly chooses (independently at random) $t − 1$ elements of $\mathbb{Z}_m, y_1, ... , y_{t−1}$.
2. $D$ computes 
$$
y_t = K − \sum_{i=1}^{t−1} y_i \mod m.
$$
3. For $1 \le i \le t$, $D$ gives the share $y_i$ to $P_i$.
...
$$
Can $t − 1$ participants compute $K$? Clearly, the first $t − 1$ participants cannot do so, since they receive $t − 1$ independent random numbers as their shares. Consider the $t − 1$ participants in the set $P \setminus\{P_j\}$, where $1\le j\le t − 1$. These $t − 1$ participants possess the shares 
$$
...
$$
By summing their shares, they can compute $K − y_j$. However, they do not know the random value $y_j$, and hence they have no information as to the value of $K$. Consequently, we have a $(t, t)$-threshold scheme.
