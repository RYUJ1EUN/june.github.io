...
1. $D$ chooses $w$ distinct, non-zero elements of $\mathbb{Z}\_{p}$, denoted $x\_{i}$, $1 \le i \le w$. For $i$, $D$ gives the value $x\_{i}$ to $P\_{i}$. The values $x\_{i}$ are public.  
Share Distribution
...
1. $D$ secretly chooses (independently at random) $t − 1$ elements of $\mathbb{Z}\_m$, 
$$
y_1, ... , y_{t−1}
$$.
2. $D$ computes 
...
$$
Can $t − 1$ participants compute $K$? Clearly, the first $t − 1$ participants cannot do so, since they receive $t − 1$ independent random numbers as their shares. Consider the $t − 1$ participants in the set $P \setminus\\{P\_{j}\\}$, where $1\le j\le t − 1$. These $t − 1$ participants possess the shares 
$$
