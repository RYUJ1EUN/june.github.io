...
We also assume that some trusted parameters $({\mathbb G}, q, g)$, generated using ${\mathcal G}(1^n)$, are published in advance, where $q$ is prime and so $\Z_q$ is a field. 
...
1. To share a secret $s$, the dealer chooses uniform $a_0 \in \Z_q$ and then shares $a_0$ as in Shamir’s scheme. 
    
    That is, the dealer chooses uniform $a_1,...,a_{t-1}\in\Z_q$ and defines the polynomial 
