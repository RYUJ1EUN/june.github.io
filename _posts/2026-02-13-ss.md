...
Let $\mathcal{P} = \left\{P_i : 1 \le i \le w\right\}$ be the set of $w$ participants. $K$ is the key set (i.e., the set of all possible keys); and $S$ is the share set (i.e., the set of all possible shares).
...
1. $D$ chooses $w$ distinct, non-zero elements of \mathbb{Z}\_p$, denoted $x\_i$, $1 \le i \le w$. For $i$, $D$ gives the value $x_i$ to $P\_i$. The values $x_i$ are public.  
Share Distribution
2. Suppose $D$ wants to share a key $K \in\mathbb{Z}\_p$. $D$ secretly chooses (independently at random) $t - 1$ elements of $\mathbb{Z}\_p$, which are denoted $a\_1, ... , a\_{t-1}$.
3. For $i$, $D$ computes $y\_i = a(x\_i)$, where
$$
    a(x) = K+\sum_{j=1}^{t-1} a_j x^j \mod p
$$
4. For $i$, $D$ gives the share $y\_i$ to $P\_i$.

In this scheme, the dealer constructs a random polynomial $a(x)$ of degree at most $t - 1$ in which the constant term is the key, $K$.Every participant $P\_i$ obtains a point $(x\_i, y\_i)$ on this polynomial.


Suppose that participants $P\_{i\_1}, \dots, P\_{i\_t}$ want to determin $K$. They know that $y\_{i\_j} = A(x\_{i\_j})$, for $1\ge i\ge t$. Since $a(x)$ has degree at most $t-1$, $a(x) = \sum\_{j=0}^{t-1} a\_j x^j$ where the coefficients are unknown and $a\_0=K$ is the key. Since $y\_{i\_j} = a(x\_{i\_j}), 1 \le j \le t$, the subset $B$ can obtain $t$ linear equations in the $t$ unknowns $a\_0, \dots, a\_{t-1}$. If the equations are linearly independent, there will be a unique solution, and $a_0$ will be revealed as the key.
...
> **THEOREM 11.3 (Lagrange interpolation formula)** _Suppose $p$ is prime, suppose $x\_1, ..., x\_{t}$ are distinct elements in $\mathbb{Z}\_p$, and suppose $y\_1, ..., y\_{t}$ are (not necessarily distinct) elements in $\mathbb{Z}\_p$. Then there is a unique polynomial $a(x)\in\mathbb{Z}\_p[x]$ having degree at most $m$, such that $a(x\_i) = y\_i, 1\le i\le t$. The polynomial $a(x)$ is as follows:
$$
...
$$
K = \sum_{j=1}^{t} \left( y_{i_j} \prod_{1 \le k \le t, k \ne j} \frac{x_{i_k}}{x_{i_k} - x_{i_j}} \right) \pmod{p},
$$
where
$$
b_j = \prod_{1 \le k \le t, k \ne j} \frac{x_{i_k}}{x_{i_k} - x_{i_j}} \pmod{p}
$$
for $1\le j\le t$. (the $b_j$'s can be precomputed, if desired, and their values are not secret.) Then we have 
$$
...
A Simplified $(t, t)$-threshold Scheme is a simplified construction for threshold schemes in the special case $w = t$. This construction will work for any key set $K = \mathbb{Z}\_m$, and it has $S = \mathbb{Z}\_m$. (For this scheme, it is not required that $m$ be prime, and it is not necessary that $m \ge w + 1$.) If $D$ wants to share the key $K \in \mathbb{Z}\_m$, he carries out the steps in Cryptosystem 11.2.


> Cryptosystem 11.2: Simplified $(t, t)$-Threshold Scheme  
1. $D$ secretly chooses (independently at random) $t − 1$ elements of $\mathbb{Z}\_m, y\_1, ... , y\_{t−1}$.
2. $D$ computes 
$$
y_t = K − \sum_{i=1}^{t−1} y_i \mod m.
$$
3. For $1 \le i \le t$, $D$ gives the share $y\_i$ to $P\_i$.
...
$$
Can $t − 1$ participants compute $K$? Clearly, the first $t − 1$ participants cannot do so, since they receive $t − 1$ independent random numbers as their shares. Consider the $t − 1$ participants in the set $P \setminus\{P\_j\}$, where $1\le j\le t − 1$. These $t − 1$ participants possess the shares 
$$
y_1, ... , y_{j−1}, y_{j+1}, ... , y_{t−1}
$$
and
$$
$K = \sum_{i=1}^{t} y_i \pmod m
$$
By summing their shares, they can compute $K − y\_j$. However, they do not know the random value $y\_j$, and hence they have no information as to the value of $K$. Consequently, we have a $(t, t)$-threshold scheme.
