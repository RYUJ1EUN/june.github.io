...
<iframe src="https://docs.google.com/viewer?url={{ '/assets/pdf/KYBER_NTT_and_Montgomery_reduction.pdf' | absolute_url }}&embedded=true" style="width:100%; height:1000px;" frameborder="0"></iframe>

<iframe src="https://docs.google.com/viewer?url={{ '/assets/pdf/KYBER_NTT_remove.pdf' | absolute_url }}&embedded=true" style="width:100%; height:1000px;" frameborder="0"></iframe>
...
<!-- <iframe src="{{ '/assets/pdf/KYBER_NTT_and_Montgomery_reduction.pdf' | relative_url }}" width="100%" height="800px" style="border: none;">
    PDF 미리보기를 불러올 수 없습니다.
    <a href="{{ '/assets/pdf/KYBER_NTT_and_Montgomery_reduction.pdf' | relative_url }}">여기</a>를 클릭해 다운로드하세요.
</iframe>

<iframe src="{{ '/assets/pdf/KYBER_NTT_remove.pdf' | relative_url }}" width="100%" height="800px" style="border: none;">
    PDF 미리보기를 불러올 수 없습니다.
    <a href="{{ '/assets/pdf/KYBER_NTT_remove.pdf' | relative_url }}">여기</a>를 클릭해 다운로드하세요.
</iframe>

<iframe src="{{ '/assets/pdf/KYBER_analysis.pdf' | relative_url }}" width="100%" height="800px" style="border: none;">
    PDF 미리보기를 불러올 수 없습니다.
    <a href="{{ '/assets/pdf/KYBER_analysis.pdf' | relative_url }}">여기</a>를 클릭해 다운로드하세요.
</iframe> -->
...
$n$-th cyclotomic polynomial $\phi_n(x)$는 primitive $n$-th roots of unity들만을 근으로 갖는 최고차항 계수가 1인 다항식임
...
즉, $x^n +1$은 primitive $2n$-th roots of unity들만을 근으로 갖는 다항식임
...
- the primitive $2n$-th root of unity $\psi_{2n}$ in $\mathbb{Z}_q$ exists.
    ...
    ※  $\mathbb{Z}\_{q}[x]/(x^{2n}-1)$의 primitive $2n$-th roots of unity $\psi\_{2n}$
    ...
    이고, $\mathbb{Z}\_{q}[x]/(x^{2n}-1)$에서 구한 $2n$개 근은 $\{ \psi\_{2n}^0, \psi\_{2n}^1, \psi\_{2n}^2, \dots, \psi\_{2n}^{2n-1} \}$이다.
    ...
    이므로 $\mathbb{Z}\_{q}[x]/(x^n-1)$의 근이 되고, $\psi\_{2n}^{2k+1}$은
    
    $$
    (\psi_{2n}^{2k+1})^n = (\psi_{2n}^{2n})^k \cdot \psi_{2n}^n = 1 \cdot (-1) = -1
    $$
    
    이므로 $\mathbb{Z}\_{q}[x]/(x^n+1)$의 근이 된다.
...
<img src="{{ '/assets/img/post/ntt_ml_kem/image.png' | relative_url }}" style="max-width: 100%; height: auto; display: block; margin: 0 auto;" alt="image">
...
<img src="{{ '/assets/img/post/ntt_ml_kem/image_1.png' | relative_url }}" style="max-width: 100%; height: auto; display: block; margin: 0 auto;" alt="image">
...
<img src="{{ '/assets/img/post/ntt_ml_kem/image_2.png' | relative_url }}" style="max-width: 100%; height: auto; display: block; margin: 0 auto;" alt="image">
<!-- ![image]({{ '/assets/img/post/ntt_ml_kem/image_2.png' | relative_url }}) -->
<!-- ![image.png](image%202.png) -->

<img src="{{ '/assets/img/post/ntt_ml_kem/image_3.png' | relative_url }}" style="max-width: 100%; height: auto; display: block; margin: 0 auto;" alt="image">
<!-- ![image]({{ '/assets/img/post/ntt_ml_kem/image_3.png' | relative_url }}) -->
<!-- ![image.png](image%203.png) -->

#### Radix-2 Fast NWC-based NTT

The Cooley-Tukey butterfly or CT butterfly`Multiplication → Addition`. 

<img src="{{ '/assets/img/post/ntt_ml_kem/14416731-2c31-4e01-9c81-ca220167df77.png' | relative_url }}" style="max-width: 50%; height: auto; display: block; margin: 0 auto;" alt="image">
...
Gentlemen-Sande butterfly or GS butterfly`Addition → Multiplication`. 

<img src="{{ '/assets/img/post/ntt_ml_kem/bc9e4c46-c1a8-4888-8f28-8fab825d7c10.png' | relative_url }}" style="max-width: 50%; height: auto; display: block; margin: 0 auto;" alt="image">
...
<img src="{{ '/assets/img/post/ntt_ml_kem/image_4.png' | relative_url }}" style="max-width: 50%; height: auto; display: block; margin: 0 auto;" alt="image">
<!-- ![image]({{ '/assets/img/post/ntt_ml_kem/image_4.png' | relative_url }}) -->
<!-- ![image.png](image%204.png) -->

<img src="{{ '/assets/img/post/ntt_ml_kem/image_5.png' | relative_url }}" style="max-width: 50%; height: auto; display: block; margin: 0 auto;" alt="image">
...
1. Kyber ring은 $\mathbb{Z}_q[x]/(x^n+1)$이므로 NWC-based NTT를 수행하기 위한 primitive $2n$-th root of unity가 필요함
    - $2n$-th root of unity $\psi_{2n}$는 $\psi_{2n}^{2n}=1$이므로
        
        $\psi_{2n}^n=-1\implies x^n+1=x^n-(\psi_{2n}^n)=0$을 만족함
        
2. Primitive $2n$-th root of unity가 존재하려면
    1. $n=2^k$
    2. $q\equiv 1\pmod{2n}$
    
    을 만족해야 함
    
3. Kyber는 $n=256=2^8$이지만 $q$에 대하여 $q\equiv 257\neq 1\pmod{512}$이므로 $2n$-th root of unity가 존재하지 않음

4. Kyber ring $\mathbb{Z}_q[x]/(x^{256}+1)$을 $(\mathbb{Z}_q[y]/(y^2-x))[x]/(x^{128}+1)$로 변환함
    1. $n'=128$에 대하여 primitive $2n$-th root of unity가 존재함
    2. FFT를 적용한 point-wise multiplication은 
        
        $(c_0 +c_1 y)(c_0' +c_1' y)\in \mathbb{Z}_q[y]/(y^2-x)$로써 수행됨
        


##### Full NWC-based NTT 적용 불가

Full NWC-based NTT는 다음을 만족해야 함

$$
n=2^k\quad\text{ and }\quad q\equiv1\;(\bmod{\;2n})
$$

특히, Full NWC-based NTT는

$$
\mathbb{Z}_q[x]/(x^n + 1) \;\cong\; \prod_{i=0}^{n-1} \mathbb{Z}_q[x]/(x - \psi_{2n}^{2\text{brv}_n(i)+1})
$$

를 만드는 과정이므로 primitive $2n$-th root of unity $\psi_{2n}$가 존재해야 적용 가능함
...
이는 $\mathbb{Z}_q$ 상에 primitive 256-th root of unity는 존재하지만, primitive 512-th root of unity는 존재하지 않음을 의미

#### Truncated NTT를 이용한 해결

##### Incomplete FFT trick (Truncated NTT)

Incomplete FFT trick은 FFT depth level $\beta$에 대하여 다항식 ring 구조를 다음과 같이 변환함

$$
\mathbb{Z}_q[x]/(x^n+1) \cong \prod_{i=0}^{n/2^\beta - 1} \mathbb{Z}_q[x] / (x^{2^\beta} - \psi_{2n/2^\beta}^{2\text{brv}(i)+1})
$$

- $\beta$는 $\log n$-level의 NTT를 $\beta$-level 이전에 마침을 의미함
...
를 만족하도록 선택하며, Kyber는 효율성을 위해 가장 작은 $\beta=1$을 선택함

$$
q=3329 \equiv 1\quad(\bmod{\;256}=\bmod{\;{2n}/2})
$$

Kyber는 $\beta=1$인 Incomplete FFT trick을 사용하므로
...
이는 Kyber ring에서 정의된 다항식 $a(x)\in \mathbb{Z}_q[x]/(x^n+1)$를 $y = x^{2^\beta} = x^2$로 치환하여 $y$에 대한 다항식
...
이는 최상위 level의 ring 구조를 $y$에 대하여 계수가 1차 다항식 $c_0+c_1 x$인 127차 다항식으로 보는 것임

Kyber는 위 구조에서 $y$에 대한 128-point Full NWC-based NTT를 수행함

$$
\mathbb{Z}_q[x]/(x^{256}+1) \to \left(\mathbb{Z}_q[x]/(x^{2} - y)\right)[y]/(y^{128} +1)
$$

또한, formal variable $x, y$를 교환하면

$$
\mathbb{Z}_q[x]/(x^{256}+1) \to \left(\mathbb{Z}_q[y]/(y^{2} - x)\right)[x]/(x^{128} +1)
$$

이므로, NTT를 적용할 다항식을 $c_0+c_1 y$ 형태의 계수를 갖는 $x$에 대한 다항식으로 표현할 수 있음

이때 $\omega=\psi_{2n/2}=\psi_{n}$이며, $\omega_i:=\omega^i$라고 할 때 다항식 $\boldsymbol{a}$는 다음으로 매핑됨
...
<img src="{{ '/assets/img/post/ntt_ml_kem/image_6.png' | relative_url }}" style="max-width: 100%; height: auto; display: block; margin: 0 auto;" alt="image">
<!-- ![image]({{ '/assets/img/post/ntt_ml_kem/image_6.png' | relative_url }}) -->
<!-- ![image.png](image%206.png) -->

#### 다항식 곱셈을 위한 NTT 선택

Kyber는 NWC-based NTT 연산을 적용해야 하므로 
...
Point-wise multiplication은

$$
a(x), b(x) \in \mathbb{Z}_q[x]/(x^2 - \omega_i)
$$

에 대하여 (formal variable $y\to x$)
...
- Algorithm line 3: len=128, 64, … ,2
    
    ⇒  Crops with $\beta=1$
    
    Full NWC-based NTT는 $2^0\sim2^7$까지 $\log 2^8=8$-level의 NTT를 수행하지만
    
    Kyber는 $\beta=1$-level인 $\text{len}=2^0$에 대한 분해를 수행하지 않음
    
- Line 8~10
    
    ⇒  Point-wise multiplication over $\mathbb{Z}_q[x]/(x^2-\zeta)$ for $\zeta=\omega_i$
